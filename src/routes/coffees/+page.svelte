<script>
    ...
    let fetchURL = `http://localhost:8000/api/coffees`;
            ...
            <div class="cards-container">
                {#each coffees as coffee}
                    <div class="coffee-card">
                        ...
                        <a class="add-to-cart" href={`/Add_to_Cart?title=${encodeURIComponent(coffee.title)}&price=${encodeURIComponent(coffee.price)}&description=${encodeURIComponent(coffee.description2)}&img=${encodeURIComponent(coffee.img)}`}> Add to cart | P{coffee.price}</a>
                    </div>
                {/each}
            </div>
        </div>
