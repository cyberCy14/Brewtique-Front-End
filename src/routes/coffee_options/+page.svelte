<script>
    ...
    let coffeeOptions = [];
    let selectedCoffeeCategory = "Black%20Coffee"; 
    let errorMessage = '';
...
<section>
    ...
    <div class="buttons-area">
        <div class="inner-div1">
            {#each Object.keys(coffeeOptions) as category (category)}
                <button class="coffees" on:click={() => selectCoffeeCategory(category)}>{category}</button>
            {/each}
        </div>
    </div>
            ...
            <div class="cards-container">
                {#each coffeeOptions[selectedCoffeeCategory] || [] as coffeeItem}
                    <div class="coffee-card">
                        ...
                        <a class="add-to-cart" href={`/Add_to_Cart?title=${encodeURIComponent(coffeeItem.title)}&price=${encodeURIComponent(coffeeItem.price)}&description=${encodeURIComponent(coffeeItem.description2)}&img=${encodeURIComponent(coffeeItem.img)}`}> Add to cart | P{coffeeItem.price}</a>
                    </div>
                {/each}
            </div>
        </div>
